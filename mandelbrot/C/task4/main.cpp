...
#pragma acc update device(image[n*block_size:block_size]) async(n)
#pragma acc parallel loop async(n)
    for(int y=n*y_block;y<(n+1)*y_block;y++) {
      for(int x=0;x<WIDTH;x++) {
	image[y*WIDTH+x]=mandelbrot(x,y);
      }
    }
#pragma acc update self(image[n*block_size:block_size]) async(n)
  }
#pragma acc wait
